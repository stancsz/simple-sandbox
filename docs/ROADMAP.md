...
- [x] **LLM Call Caching**: Implement a caching layer (`src/llm/cache.ts`) using a file-based or Redis backend to intercept identical LLM prompts and serve cached responses, thus avoiding redundant API calls. Integrate metrics (`llm_cache_hit`, `llm_cache_miss`) into the Health Monitor.
    ...
    - ✅ Validated: End-to-end tests and performance benchmarks in `tests/integration/llm_cache_validation.test.ts` verify a significant reduction in duplicate API calls and token consumption.
- [x] **Batch Prompt Consolidation**: Bundle routine daily or hourly strategic scans into single context windows to maximize contextual token usage.
    - ✅ Implemented `BatchExecutor` logic in `src/batch/batch_orchestrator.ts`.
    - ✅ Extended Scheduler MCP to register routine tasks with `is_routine` and `frequency`.
    - ✅ Created `execute_batch_routines` tool to manually trigger strategic batches.
    - ✅ Validated via `tests/integration/batch_processing_validation.test.ts`.
- [x] **Adaptive Model Routing**: Dynamically route LLM requests (e.g., using Haiku/Flash for parsing and Opus/Reasoner for complex reasoning) based on task complexity scores generated by the `business_ops` MCP.
    - ✅ Implemented `evaluate_task_complexity` tool in `business_ops` MCP using lightweight models and heuristics.
